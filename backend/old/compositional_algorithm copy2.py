...
def replace(
    ...
    Comments:
        - Assumption: Important Lables must contain a!?, b!?,... for the ageents interactions.
        - Instead of replaceing all the time, we save the subnets and build the final net at the end.
...
def compositional_discovery(
    ...
    **algorithm_kwargs: dict,
    ...
    multi_agent_net = interface_pattern.__deepcopy__()
    ...
    with tqdm(total=len(unique_agents), desc="Processing Agents") as pbar:
        ...
        for i, agent in enumerate(unique_agents):
            ...
            if check_refinement:
                ...
                copy_net = multi_agent_net.__deepcopy__()
                ...
                # Update copy_net to the latest transformed net
                multi_agent_net = copy_net
