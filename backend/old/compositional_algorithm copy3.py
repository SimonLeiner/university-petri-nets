...
def is_valid(
    ...
    end_net: PetriNet,
    markings: dict[Marking, Marking, Marking, Marking],
) -> bool:
    ...
    Args:
        ...
        markings (dict): The initial and final markings of the agent GWF-net.
    ...
    # check length of the markings is the is max the gwef net
    valid = valid and len(markings["i_s_init"]) <= len(markings["g_a_init"])
    return valid and len(markings["i_s_final"]) <= len(markings["g_a_final"])
...
def is_refinement(  # noqa: C901
    ...
    with tqdm(total=None, desc="Processing Queue") as pbar:
        while queue:
            ...
            print(current_net.places)
            print(markings["i_s_init"])
            print(markings["i_s_final"])
            view_petri_net(current_net, markings["i_s_init"], markings["i_s_final"], format="png")
            ...
            for place in current_net.places:
                ...
                for place_transformation in place_transformations:
                    ...
                    # if P1 adjust for the initial marking and update the marking
                    if isinstance(place_transformation(), transformations[0]):
                        transformed_net, transformed_init_marking = (
                            place_transformation.refine(
                                place,
                                net_copy,
                                markings["i_s_init"],
                            )
                        ...
                        print("---")
                        print(markings["i_s_init"])
                        print(transformed_init_marking)
                        markings["i_s_init"] = transformed_init_marking
                    else:
    ...
    **algorithm_kwargs: dict,
    ...
    with tqdm(total=len(unique_agents), desc="Processing Agents") as pbar:
        ...
        for i, agent in enumerate(unique_agents):
            ...
            gwf_agent_net, gwf_agent_initial_marking, gwf_agent_final_marking = (
                discover(
                    input_log_path,
                    algorithm,
                    **algorithm_kwargs,
            ...
            (
                ...
                interface_subset_initial_marking,
                interface_subset_final_marking,
            ) = interface_pattern.get_net(f"A{i+1}")
            subnets[f"A{i}"] = interface_subset_pattern

            # connect the marking as a tuple
            markings = {
                "i_s_init": interface_subset_initial_marking,
                "i_s_final": interface_subset_final_marking,
                "g_a_init": gwf_agent_initial_marking,
                "g_a_final": gwf_agent_final_marking,
            }
