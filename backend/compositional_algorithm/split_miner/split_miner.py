...
def split_miner(  # noqa: C901, PLR0913
    input_log_path: str,
    ...
    jar_path = (
        "/app/backend/compositional_algorithm/split_miner/split-miner-1.7.1-all.jar"
    )

    # create output path from input path
    dir_name = os.path.dirname(input_log_path)  # noqa: PTH120
    base_name, _ = os.path.splitext(os.path.basename(input_log_path))  # noqa: PTH119, PTH122
    new_base_name = base_name + "_split_mined.bpmn"
    output_path = os.path.join(dir_name, new_base_name)  # noqa: PTH118
    ...
    while True:
        ...
        if time.time() - start_time >= timeout:
            msg = f"File {output_path} not found."
